...
async function runShards(runnerOptions: RunnerOptions, baseCoverageDir: string): Promise<number> {
  ...
  if (runnerOptions.coverage.enabled) {
    console.log(
      `[test:sharded] coverage mode uses a single deterministic run across ${String(allFiles.length)} files`,
    );
    const allTestsShard: ShardConfig = {
      name: 'coverage-all',
      files: [...allFiles],
      estimatedCost: allFiles.reduce((sum, filePath) => sum + estimateFileCost(filePath), 0),
    };
    return await runShardWithOptions(allTestsShard, runnerOptions, baseCoverageDir);
  }
  ...
  const results = await Promise.all(
    shards.map(async (shard) => {
      const exitCode = await runShardWithOptions(shard, runnerOptions, null);
      return {
        name: shard.name,
        exitCode,
      };
