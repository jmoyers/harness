...
- A first interactive CLI/TUI entrypoint now exists as `harness nim` (with direct script alias `bun run nim:tui`) via `packages/nim/src/cli.ts` for end-to-end validation of run/replay/queue/abort flows.
