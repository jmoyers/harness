...
- Startup lifecycle composition is moving into a class-based `StartupOrchestrator` (`src/services/startup-orchestrator.ts`) so runtime delegates startup settle/probe/resume/hydration/initial-activation wiring through one seam instead of coordinating multiple startup service instances inline.
