...
- Task-pane selection/editor transitions are moving into a class-based `TaskPaneSelectionActions` (`src/services/task-pane-selection-actions.ts`) so runtime delegates draft/task/repository focus-selection state changes instead of keeping task-pane transition handlers inline.
