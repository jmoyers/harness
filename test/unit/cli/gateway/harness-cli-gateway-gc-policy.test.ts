...
void serialCliTest(
  'harness gateway gc removes named sessions older than one week and keeps recent sessions',
  async () => {
...
void serialCliTest(
  'harness gateway gc skips live named sessions even when their artifacts look stale',
  async () => {
    const workspace = createWorkspace();
