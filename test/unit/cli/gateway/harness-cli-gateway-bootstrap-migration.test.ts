...
void serialCliTest(
  'harness gateway list reports no targets when no gateway records or daemons are present',
  async () => {
    const workspace = createWorkspace();
    try {
...
void serialCliTest(
  'harness auto-migrates legacy local .harness record path to global runtime root on first run',
  async () => {
    const workspace = createWorkspace();
...
void serialCliTest(
  'harness migrates legacy local config when global config is only bootstrapped default',
  async () => {
    const workspace = createWorkspace();
    try {
...
void serialCliTest(
  'harness gateway start rejects local workspace .harness state db path',
  async () => {
    const workspace = createWorkspace();
    const port = await reservePort();
...
void serialCliTest(
  'harness gateway start adopts an already-reachable daemon when gateway record is missing',
  async () => {
    const workspace = createWorkspace();
    const port = await reservePort();
    const runtimeRoot = workspaceRuntimeRoot(workspace);
