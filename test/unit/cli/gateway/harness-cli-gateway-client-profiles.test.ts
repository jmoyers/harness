...
void serialCliTest(
  'harness profile writes client and gateway CPU profiles in isolated session paths',
  async () => {
...
void serialCliTest(
  'harness profile start/stop writes gateway CPU profile to global profiles path for the target session',
  async () => {
    const workspace = createWorkspace();
