...
let websocketMockLock: Promise<void> = Promise.resolve();

async function withWebSocketMockLock<T>(run: () => Promise<T>): Promise<T> {
  let releaseLock: () => void = () => {};
  const nextLock = new Promise<void>((resolve) => {
    releaseLock = resolve;
  });
  const previousLock = websocketMockLock;
  websocketMockLock = previousLock.then(() => nextLock);
  await previousLock;
  try {
    return await run();
  } finally {
...
test('inspector websocket connect handles open, error, and timeout outcomes', async () => {
  await withWebSocketMockLock(async () => {
