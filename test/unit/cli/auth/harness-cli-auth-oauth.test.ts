...
void concurrentCliTest('harness auth login github requires HARNESS_GITHUB_OAUTH_CLIENT_ID', async () => {
...
void concurrentCliTest('harness auth login linear completes oauth callback flow and stores tokens', async () => {
  const workspace = createWorkspace();
...
void concurrentCliTest('harness auth refresh linear updates expired oauth token and auth logout clears secrets', async () => {
  const workspace = createWorkspace();
