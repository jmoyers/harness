...
let perfCoreTestLock: Promise<void> = Promise.resolve();
let ptyHostTestLock: Promise<void> = Promise.resolve();

async function withPerfCoreTestLock<T>(run: () => Promise<T>): Promise<T> {
  let releaseLock: () => void = () => {};
  const nextLock = new Promise<void>((resolve) => {
    releaseLock = resolve;
  });
  const previousLock = perfCoreTestLock;
  perfCoreTestLock = previousLock.then(() => nextLock);
  await previousLock;
  try {
    return await run();
  } finally {
...
interface PtyHostTestOptions {
  timeout?: number;
  retry?: number;
  repeats?: number;
}

type PtyHostTestCallback = () => void | Promise<unknown>;

async function withPtyHostTestLock<T>(run: () => Promise<T>): Promise<T> {
  let releaseLock: () => void = () => {};
  const nextLock = new Promise<void>((resolve) => {
    releaseLock = resolve;
  });
  const previousLock = ptyHostTestLock;
  ptyHostTestLock = previousLock.then(() => nextLock);
  await previousLock;
  try {
    return await run();
  } finally {
...
function normalizePtyHostTestOptions(
  options: number | PtyHostTestOptions | undefined,
): PtyHostTestOptions {
...
function serialPtyTest(
  name: string,
  fn: PtyHostTestCallback,
  options?: number | PtyHostTestOptions,
): void {
  test(name, normalizePtyHostTestOptions(options), async () => {
    await withPtyHostTestLock(async () => {
      await fn();
    });
  });
}

void serialPtyTest('pty-host transparently echoes input using cat', async () => {
...
void serialPtyTest('pty-host forwards shell output and exits cleanly', async () => {
  const session = startPtySession({
    command: '/bin/sh',
...
void serialPtyTest('pty-host runs command in provided cwd', async () => {
...
void serialPtyTest('pty-host forwards stderr output through data stream', async () => {
  const session = startPtySession({
    command: '/bin/sh',
...
void serialPtyTest('pty-host resize updates terminal size seen by shell', async () => {
  const session = startPtySession({
    command: '/bin/sh',
    commandArgs: ['-i'],
    env: {
...
void serialPtyTest('pty-host applies initial terminal size before first command', async () => {
  const session = startPtySession({
    command: '/bin/sh',
    commandArgs: ['-i'],
    env: {
...
void serialPtyTest('pty-host close command exits default interactive shell session', async () => {
  const session = startPtySession();
  session.close();
  const exit = await waitForExit(session);
...
void serialPtyTest('pty-host emits error when helper executable cannot be launched', async () => {
  const session = startPtySession({
...
void serialPtyTest('pty-host preserves terminal control sequences in output stream', async () => {
...
void serialPtyTest(
  ...
  async () => {
    await withPerfCoreTestLock(async () => {
      ...
      try {
        ...
        await waitForCondition(
          () => {
            try {
              return readPerfRecords(perfFilePath).some(
                (record) => record.type === 'event' && record.name === 'pty.stdout.chunk',
              );
            } catch {
              return false;
            }
          },
          'pty.stdout.chunk perf event',
          2000,
        );
