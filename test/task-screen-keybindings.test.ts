...
void test('task screen keybindings require explicit ctrl/cmd bindings', () => {
  ...
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from('\u001b[97;9u', 'utf8'), bindings),
    'mux.home.editor.line.start',
  );
  assert.equal(detectTaskScreenKeybindingAction(Buffer.from([0x01]), bindings), null);
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from('\u001b[103;9u', 'utf8'), bindings),
    null,
  );
  assert.equal(
    detectTaskScreenKeybindingAction(Buffer.from([0x07]), bindings),
    'mux.home.repo.dropdown.toggle',
  );
});

void test('task screen keybindings deduplicate repeated normalized strokes', () => {
  const bindings = resolveTaskScreenKeybindings({
    'mux.home.repo.dropdown.toggle': ['ctrl+g', 'control+g'],
  });
